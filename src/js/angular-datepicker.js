    ...
    function link($scope, element, attr) {
      ...
      function setDaysInMonth() {
        ...
        if(firstDayWeekNumber !== 0 ){
          ...
          for(var i = previousMonthlastDayWeekDay; i >= 0; i--) {
      ...
      function paginateYears() {
        ...
        for (var i = yearsToPrepend; i > 0; i -= 1) {
          theNewYears.push(Number(startingYear) - i);
        }

        for (var i = 0; i < yearsToAppend; i += 1) {
          theNewYears.push(Number(startingYear) + i);
        }
      ...
      function generateYears(pace, baseYear) {
        ...
        for (var i = pace; i > 0; i -= 1) {
          theNewYears.push(Number(baseYear) - i);
        }

        for (var i = 0; i < pace; i += 1) {
          theNewYears.push(Number(baseYear) + i);
        }
        return theNewYears;
      }
      ...
      function isSelectableDate(date) {
        ...
        if (hasDisabledDates) {
          for (var i = 0; i <= dateDisabledDates.length; i += 1) {
      ...
      function isSelectableMinYear(year) {
        if(!dateMinLimit) return true;
        return year >= new Date($scope.dateMinLimit).getFullYear();
      }
      ...
      function getDateSet() {
        ...
        if(dateSet) {
          ...
          if(dateSet.toString() !== 'Invalid Date') {
            return new Date(dateSet.getTime());
      ...
      $scope.$watch('dateMinLimit', function(newVal, oldVal) {
        if (newVal !== oldVal) {
          setDaysClass();
          var currentDate = new Date($scope.year, monthNumber, $scope.day);
          var isValid = isSelectableMinDate(currentDate);

          if (!isValid) {
            resetToMinDate();
            $scope.day = new Date(newVal).getDate();
            setInputValue();
          }
        }
      })
